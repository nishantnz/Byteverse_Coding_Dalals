...
class MRIScan extends ConsumerStatefulWidget {
  const MRIScan({
    super.key,
    required this.type,
  });
  final UserTypeEnum type;
  static const routeName = '/mri-scan';

  @override
  ConsumerState<MRIScan> createState() => _MRIScan();
}

class _MRIScan extends ConsumerState<MRIScan> {
  File? image;
  String? output;
  String? result;
  String? patientName;
  ...
  void getResult() async {
    ...
    var url = Uri.parse(
      'https://getprediction-5eh7jedtsq-el.a.run.app/mri',
    );
    // open a bytestream
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'MRI Scan by ${widget.type == UserTypeEnum.doctor ? 'Doctor' : 'Patient'}',
        ),
      ),
