...
class SkinDiseaseScan extends ConsumerStatefulWidget {
  const SkinDiseaseScan({
    super.key,
    required this.type,
  });
  final UserTypeEnum type;
  static const routeName = '/skin-disease-scan';

  @override
  ConsumerState<SkinDiseaseScan> createState() => _SkinDiseaseScan();
}

class _SkinDiseaseScan extends ConsumerState<SkinDiseaseScan> {
  File? image;
  String? output;
  String? result;
  String? patientName;
  ...
  void getResult() async {
    ...
    var url = Uri.parse(
      'https://getprediction-5eh7jedtsq-el.a.run.app/skin',
    );
    // open a bytestream
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Skin Disease Scan by ${widget.type == UserTypeEnum.doctor ? 'Doctor' : 'Patient'}',
        ),
      ),
      body: SafeArea(
        child: Center(
