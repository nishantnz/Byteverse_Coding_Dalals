...
class CTScan extends ConsumerStatefulWidget {
  const CTScan({
    super.key,
    required this.type,
  });
  final UserTypeEnum type;
  static const routeName = '/ct-scan';

  @override
  ConsumerState<CTScan> createState() => _CTScan();
}

class _CTScan extends ConsumerState<CTScan> {
  File? image;
  String? output;
  String? result;
  String? patientName;
  ...
  void getResult() async {
    ...
    var url = Uri.parse(
      'https://getprediction-5eh7jedtsq-el.a.run.app/ct',
    );
    // open a bytestream
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'CT Scan by ${widget.type == UserTypeEnum.doctor ? 'Doctor' : 'Patient'}',
        ),
      ),
      body: SafeArea(
        child: Center(
